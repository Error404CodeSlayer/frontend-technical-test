...
const ConversationsList: React.FC<ConversationsListProps> = ({
    ...
    const getConversationIdForUser = (currentUser: string, conversations: Conversation[]) => {
        const conversationsWithUserName: { userName: string, conversationId: number }[] = [];
        
        for (const conversation of conversations) {
            if (currentUser === conversation.recipientNickname) {
                conversationsWithUserName.push({userName: conversation.senderNickname, conversationId: conversation.id});
            } else {
                conversationsWithUserName.push({userName: conversation.recipientNickname, conversationId: conversation.id})
            }
        }
    ...
    const conversationIdForUsers = getConversationIdForUser(user.nickname, conversations);
    ...
    return (
        ...
        {conversationIdForUsers.map((conversationIdForUser) => (
            <Link key={conversationIdForUser.conversationId} href={`/conversation/${conversationIdForUser.conversationId}`}>
                <div
